#{'Mohan': 84, 'Shruti': 91, 'Harsha': 100,'Ansh': 68,'Ravi': 71, 'Spoorthi': 79,'Fred': 55, 'George': 63,'Raj': 44,'Praveen': 37}

import turtle
import pandas as pd
s=pd.read_excel(r'Grades.xlsx')
...
for i in range(0,len(names)):
    if s['marks'][i]>=65 and s['marks'][i]<80:
        cls1_students[s['student'][i]]=marks[i]

for i in range(0,len(names)):
    if s['marks'][i]>=45 and s['marks'][i]<65:
        cls2_students[s['student'][i]]=marks[i]

for i in range(0,len(names)):
    if s['marks'][i]>=36 and s['marks'][i]<45:
